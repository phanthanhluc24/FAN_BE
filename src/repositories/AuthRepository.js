...
class AuthRepository {
    ...
    async verificationCode(req, res) {
        ...
        try {
            if (validator.isEmpty(codeToken) || validator.isEmpty(code)) {
                return res.status(201).json({ status: 401, message: "Mã xác thực không được bỏ trống" })
            }
            ...
            if (decode.code == code) {
                ...
                return res.status(201).json({ status: 401, message: "Mã xác thực không đúng" })
            }
        } catch (error) {
            if (error.name == "TokenExpiredError") {
                return res.status(201).json({ status: 401, message: "Mã code đã hết hiệu lực" })
            }
        }
    }
    ...
    async reSendVerificationCode(req, res) {
        ...
        try {
            ...
            if (!user) {
                return res.status(201).json({ status: 401, message: "Tài khoản không tìm thấy" })
            }
            const code = Math.floor(1000 + Math.random() * 9000)
    ...
    async resetNewPassword(req,res){
        ...
        try {
            if (validator.isEmpty(newPassword)|| validator.isEmpty(confirmPassword)) {
                return res.status(201).json({status:401,message:"Mật khẩu không được bỏ trống"})
            }
            const options = {
                minLength: 8,
                minLowercase: 1,
            ...
            if(newPassword !=confirmPassword){
                return res.status(201).json({status:401,message:"Xác thực mật khẩu không khớp"})
            }
            ...
            if (error.name=="TokenExpiredError") {
                return res.status(201).json({ status: 401, message: "Thời gian lấy lại mật khẩu đã hết hạn" })
            }
        }
    }
    ...
    async refreshToken(req,res){
        ...
        try {
            if (validator.isEmpty(refreshToken)) {
                return res.status(201).json({status:401,message:"Chưa cung cấp mã token"})
            }
